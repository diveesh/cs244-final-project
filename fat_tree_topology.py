        ...
        if i == L:
            ...
            for m in range(num_groups): # group num
                for j in range(num_switches_per_group): # index within group
                    ...
                    for n in range(p):
                        if i == 0: #L0 to L1
                            to_switch_num = n_switches_per_layer + (m / p) * p + n
                        else: #L1 to L2
                            to_switch_num = 2 * n_switches_per_layer + ((m * num_switches_per_group + j) % p) * p + n #(i + 1) * n_switches_per_layer + j * p + n
                        # print i, switch_num, to_switch_num
